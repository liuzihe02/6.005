...
public class RulesOf6005 {
    ...
    public static boolean mayUseCodeInAssignment(boolean writtenByYourself,
        ...
        // can always use code written by yourself
        if (writtenByYourself) {
            return true;
        }
        // cannot use code that is private
        if (!availableToOthers) {
            return false;
        }
        // cannot use code thats already written as solutions for 6.005
        if (writtenAsCourseWork) {
            return false;
        }
        // must always cite your source
        if (!citingYourSource) {
            return false;
        }
        // if need implementation then must cannot use this code
